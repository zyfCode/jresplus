		            	...
		            	if(operate=="add"){
		                   ...
		                   if (Horn.getComp(table).getSelecteds().length==0){
			        		   ...
			        		   rowData = Horn.getComp(table).getSelecteds().length && (Horn.getComp(table).getSelecteds())[0];   
			        		    //股东信息删除
			            		messageobject="配置信息";
			            		url="/am/am/system/sysconfig/del/operation.json?l_function_no="+functionIds["sysconfig"]["del"]+"&&dotype=3";			           
...
//			     	        			}
		...
		commit:function(operate,form,win){
			...
			if(result)
			{
				...
				var url;
				if(operate=="add"){//添加
						url="/am/am/system/sysconfig/doAdd/operation.json?l_function_no="+functionIds["sysconfig"]["add"]+"&&dotype=1";
				}else if(operate=="edit"){//修改
						url="/am/am/system/sysconfig/doEdit/operation.json?l_function_no="+functionIds["sysconfig"]["edit"]+"&&dotype=2";
				}else{
						url="";
				}
