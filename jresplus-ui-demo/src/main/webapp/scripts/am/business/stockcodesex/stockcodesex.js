...
var stockcodesex2 = {
	...
	viewDetail : function(params){ 	
		var show_type=Horn.getCompById("show_type").getValue(),
			...
			url = constants.stockcode.url[show_type] + "?vc_stock_code=" + params;
		...
		$.post("/am/am/init/list.json", null, function(data) {
			if (data!=null) {					
	            if(data.l_operate_flag==0){	        		
	        		...
	        		var url = "/am/am/business/stockcodesex/add.htm";
	        		if (type == 1) {
	        			window.parent.parent.Horn.Frame.openMenu('402012', url, title, 'inbox');
	        		} else {
						...
						if (type == 2) {
							title = titbefore+"变更";
							url = "/am/am/business/stockcodesex/edit.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('402013', url, title, 'inbox');
						} else if (type == 3) {
							title = titbefore+"撤销";
							url = "/am/am/business/stockcodesex/revoke.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('402014', url, title, 'inbox');
						}else if (type == 4) {
							...
							if(iframelength!=0){
								TDialog.Msg.error("提示","还款计划操作界面已打开,请先完成操作!",function(){//ok 
									window.parent.parent.Horn.Frame.openMenu('402016', url, title, 'inbox');
								}); 
							}else{
								window.parent.parent.Horn.Frame.openMenu('402016', url, title, 'inbox');
							}
		...
		$.post("/am/am/init/list.json", null, function(data) {
			if (data!=null) {					
	            if(data.l_operate_flag==0){	        		
	            	...
	            	var titbefore="物权投资-";
	        		var title = titbefore+"新增";
	        		var url = "/am/am/business/interest/add.htm";
	        		if (type == 1) {
	        			window.parent.parent.Horn.Frame.openMenu('404012', url, title, 'inbox');
	        		} else {
	        			var grid = common_operate.checkSelectedLength();
	        			if (!grid) {
	        				return grid;
						...
						if (type == 2) {
							title = titbefore+"变更";
							url = "/am/am/business/interest/edit.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('404013', url, title, 'inbox');
						} else if (type == 3) {
							title = titbefore+"撤销";
							url = "/am/am/business/interest/revoke.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('404014', url, title, 'inbox');
						}else if (type == 4) {	
							title = titbefore+"还款计划";
							url = "/am/am/business/stockcodesex/repayplanlist.htm?vc_stock_code="
									+ grid[0].vc_stock_code+"&&l_rate_id=1";
							//处理弹开界面已打开问题
							var iframelength=$('iframe[tabid=404016]', parent.parent.document).length;
							if(iframelength!=0){
								TDialog.Msg.error("提示","还款计划操作界面已打开,请先完成操作!",function(){//ok 
									window.parent.parent.Horn.Frame.openMenu('404016', url, title, 'inbox');
								}); 
							}else{
								window.parent.parent.Horn.Frame.openMenu('404016', url, title, 'inbox');
							}
		...
		$.post("/am/am/init/list.json", null, function(data) {
			if (data!=null) {					
	            if(data.l_operate_flag==0){	        		
	            	...
	            	var titbefore="债权投资-";
	        		var title = titbefore+"新增";
	        		var url = "/am/am/business/credit/add.htm";
	        		if (type == 1) {
	        			window.parent.parent.Horn.Frame.openMenu('405012', url, title, 'inbox');
	        		} else {
	        			var grid = common_operate.checkSelectedLength();
	        			if (!grid) {
	        				return grid;
						...
						if (type == 2) {
							title = titbefore+"变更";
							url = "/am/am/business/credit/edit.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('405013', url, title, 'inbox');
						} else if (type == 3) {
							title = titbefore+"撤销";
							url = "/am/am/business/credit/revoke.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('405014', url, title, 'inbox');
						}else if (type == 4) {
							title = titbefore+"还款计划";
							url = "/am/am/business/stockcodesex/repayplanlist.htm?vc_stock_code="
									+ grid[0].vc_stock_code+"&&l_rate_id=1";
							//处理弹开界面已打开问题
							var iframelength=$('iframe[tabid=405016]', parent.parent.document).length;
							if(iframelength!=0){
								TDialog.Msg.error("提示","还款计划操作界面已打开,请先完成操作!",function(){//ok 
									window.parent.parent.Horn.Frame.openMenu('405016', url, title, 'inbox');
								}); 
							}else{
								window.parent.parent.Horn.Frame.openMenu('405016', url, title, 'inbox');
							} 
		...
		$.post("/am/am/init/list.json", null, function(data) {
			if (data!=null) {					
	            if(data.l_operate_flag==0){	        		
	            	...
	            	var titbefore="存款-";
	        		var title = titbefore+"新增";
	        		var url = "/am/am/business/deposit/add.htm";
	        		if (type == 1) {
	        			window.parent.parent.Horn.Frame.openMenu('406012', url, title, 'inbox');
	        		} else {
	        			var grid = common_operate.checkSelectedLength();
	        			if (!grid) {
	        				return grid;
						...
						if (type == 2) {
							title = titbefore+"变更";
							url = "/am/am/business/deposit/edit.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('406013', url, title, 'inbox');
						} else if (type == 3) {
							title = titbefore+"撤销";
							url = "/am/am/business/deposit/revoke.htm?vc_stock_code="
									+ grid[0].vc_stock_code + "&vc_product_id="
									+ grid[0].vc_product_id;
							window.parent.parent.Horn.Frame.openMenu('406014', url, title, 'inbox');
						}else if (type == 4) {
							title = titbefore+"还款计划";
							url = "/am/am/business/stockcodesex/repayplanlist.htm?vc_stock_code="
									+ grid[0].vc_stock_code+"&&l_rate_id=1";
							//处理弹开界面已打开问题
							var iframelength=$('iframe[tabid=406016]', parent.parent.document).length;
							if(iframelength!=0){
								TDialog.Msg.error("提示","还款计划操作界面已打开,请先完成操作!",function(){//ok 
									window.parent.parent.Horn.Frame.openMenu('406016', url, title, 'inbox');
								}); 
							}else{
								window.parent.parent.Horn.Frame.openMenu('406016', url, title, 'inbox');
							}
	...
	doAdd : function(urlType,committype) {
		...
		var url = "/am/am/business/stockcodesex/" + urlType +"doAdd/operation.json?l_function_no="
				+ functionIds.stockcodesex.add + "&workflow_step="
				+ workflowStep.launch + "&business_type=htxz&c_business_class=1";
		if (form.isValid()) {
	...
	equity_Add : function(type) {
		...
		var url = "/am/am/business/stockcodesex/operation_quity.json?l_function_no="
	...
	updateInit4 : function(type) {
		...
		if (type == "_edit") {
			...
			//common_operate.geteditlist("l_org_cust_id"+ type,"loanrival","1000185");
			Horn.getCompById("en_contract_balance_x" + type).setValue($("#en_contract_balance"+ type + " > .u-typefield-capital").text());
			stockcodesex2.queryCpdm(type,"4");
			Horn.getComp("vc_project_code").setReadonly(true);
...
//    		}
	...
	doEdit : function(type,urlType) {
		...
		var url = "/am/am/business/stockcodesex/" + type + urlType + "/operation.json?l_function_no="
