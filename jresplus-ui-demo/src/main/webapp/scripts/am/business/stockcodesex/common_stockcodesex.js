...
var common_stockcodesex = {
	...
	checkCpdmDiv : function(){
		var div = $("div .stockcodesex_title:contains('基金信息')");
		div.html("项目信息");				
		Horn.getComp("vc_project_code").setDisabled(false);
		Horn.getComp("vc_project_type").setDisabled(false);
		Horn.getComp("c_project_properties").setDisabled(false);
		Horn.getComp("vc_project_manager").setDisabled(false);
		Horn.getComp("en_project_money").setDisabled(false);
		Horn.getComp("vc_project_code").show();
		Horn.getComp("vc_project_type").show();
		Horn.getComp("c_project_properties").show();
		Horn.getComp("vc_project_manager").show();
		Horn.getComp("en_project_money").show();
	...
	doAdjustCommit : function(type) {
		...
		var url = "/am/am/business/stockcodesex/" + type + "/adjust_commit.json?l_function_no="
...
//				}
	...
	showStockrate : function(committype) {
		...
		if(committype=="htadd"){
			...
			//处理用于金融产品投资的类型判断
			if(show_type=="1"||show_type=="3"||show_type=="5"||show_type=="9"||show_type=="a"){
				show_type="F";
			}
...
//				}
	...
	doRateCommit : function(type,committype) {
		...
		dialog.dialog("open");
		var url = "/am/am/business/stockcodesex/" + type + "/rate_commit.json?l_function_no="
	...
	fareRateQuery : function(type){
		...
		if (data!=null) {					
	       if(data.l_operate_flag==0){	        		
	            ...
	            url = "/am/am/business/stockcodesex/fareRate.htm?vc_stock_code="+ grid[0].vc_stock_code
...
//		Horn.getCompById("l_begin_date").setValue(grid[0].l_begin_date);
	...
	doFareRateCommit : function() {
		...
		dialog.dialog("open");
		var url = "/am/am/business/stockcodesex/" + htype +"/fareRate_commit.json?l_function_no="
		...
		query_operate.ajax(url, data, function(result) {
			if (result instanceof Array && result) {
				if(!constants.funds.invest[type] && result.length < 1){//收款
					...
					url = "/am/am/business/stockcodesex/queryCpdm.json?l_function_no=" + functionIds.stockcodesex.query_cpdm_q;
