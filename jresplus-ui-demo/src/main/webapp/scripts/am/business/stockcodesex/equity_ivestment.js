...
var equity_investment = {
	...
	getButtonListHtml : function(data){
		...
		for(var i=0,j=targetButtonArr.length;i<j;i++){
			...
			if(type=='gqtz'||type=='jctz'||type=='wqtz'||type=='zqtz'||type=='ck'){
				if(s!='2'){
...
//		无抵扣金额的写法
	...
	equity_Add : function(type) {
		...
		var url = "/am/am/business/stockcodesex/" + type + "/operation_quity.json?l_function_no="
	...
	o32Continue : function(){
		var h_l_serial_no = $("#h_l_serial_no").val(),
			...
			url = "/am/am/business/stockcodesex/" + type + "/operation_quity.json?l_function_no="
				+ functionIds.stockcodesex.equity,
...
//			        		}, "json");   
		...
		del:function(tablename){
        	...
        	if(tablename==''){
        		var vc_stock_code=Horn.getCompById("repayplanvc_stock_code").getValue();
        		url="/am/am/business/stockcodesex/repayplanoperation.json?l_function_no="+functionIds["repayplan"]["operdeal"];
    		            	...
    		            	if (Horn.getCompById(tablename).getSelecteds().length==0){
    		            		Horn.Tip.warn("请选择一条记录！");
    		    			     ...
    		    			     if(tablename=="repayplanTable"){
  	    		    			  ...
  	    		    			  url="/am/am/business/stockcodesex/repayplandel.json?l_function_no="+functionIds["repayplan"]["operdeal"];
...
//  				    						}
    		    			    	 ...
    		    			    	 rowData = Horn.getCompById(tablename).getSelecteds().length && (Horn.getComp(tablename).getSelecteds())[0];  
    	    		    			  url="/am/am/business/stockcodesex/repayplanoperation.json?l_function_no="+functionIds["repayplan"]["detaildeal"];
    				    		         //删除单条收益明细		    			    
...
//    						    						}
		...
		delalldetail:function(){
				...
				var	url="/am/am/business/stockcodesex/repayplanoperation.json?l_function_no="+functionIds["repayplan"]["detaildeal"];  
				if(closeflag=='0'){
					//删除本次操作的收益明细
...
//							}
		...
		automake:function(){
			TDialog.Msg.confirm("提示","自动生成还款计划功能会覆盖现有数据并重新生成,确定继续操作？",function(){
		        var vc_stock_code=Horn.getCompById("repayplanvc_stock_code").getValue();
	            dialog.dialog("open");
				$.post("/am/am/business/stockcodesex/repayplanoperation.json?l_function_no="+functionIds["repayplan"]["automake"]+"&&l_action_in=1&&vc_stock_code="+vc_stock_code, null, function(data) {
					common_operate.endProgress();
				    if (data == "ok") {
		...
		repayplanCommit:function(tablename){
		    ...
		    //还款计划提交
		    	...
		    	url="/am/am/business/stockcodesex/repayplanoperation2.json?l_function_no="+functionIds["repayplan"]["detailconfirm"];
